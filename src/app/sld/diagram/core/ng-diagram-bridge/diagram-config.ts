...
export function buildDiagramConfig(deps: {
  readonly modeService: DiagramModeService;
  readonly registry: SymbolRegistryService;
  readonly modelService: NgDiagramModelService;
}): NgDiagramConfig {
  const { modeService, registry, modelService } = deps;
  ...
  return {
    ...
    snapping: {
      ...
      computeSnapForNodeSize: () => ({ width: GRID, height: GRID }),
      defaultResizeSnap: { width: GRID, height: GRID },
    },
    resize: {
      defaultResizable: true,
      // Junction nodes are tiny anchors — exempt from the regular 2-grid minimum.
      getMinNodeSize: (node: Node) => {
        if (node.type === SLD_JUNCTION_NODE_TYPE) {
          return { width: SLD_JUNCTION_SIZE_PX, height: SLD_JUNCTION_SIZE_PX };
        }
        return { width: GRID * 2, height: GRID * 2 };
      },
    },
    nodeRotation: {
      // Native rotation sets node.angle which hides the resize adornment — we rotate via data.orientation + CSS transform.
    ...
    linking: {
      ...
      validateConnection: (
        ...
        if (!modeService.current().allowsConnections) return false;
    ...
    zoom: {
      // Start at 100% — the toolbar "fit" button is the explicit recentre.
