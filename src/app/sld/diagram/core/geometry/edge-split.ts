...
// Orthogonal-polyline geometry: find where a point lands on an edge
// (findEdgeSplitHit), then carve that edge's polyline into two halves at the hit
// (splitPolylineAt). Turning a hit into a junction is the caller's job.

// A landing spot on an existing edge: which edge, the grid-snapped point on it,
// and the index of the segment that was hit.
