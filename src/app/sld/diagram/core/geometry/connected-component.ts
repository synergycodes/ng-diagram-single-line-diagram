// Connected-component search over the inferred geometric graph: which nodes are
// electrically joined to a starting node. Connections aren't stored as edges —
// they're derived here from coincident points and points lying on wires.
...
// BFS over the connection graph from `startId`, returning every reachable node id.
// Lead mid-segment hits are intentionally NOT counted — a lead is the symbol's own
// outline, not an independent conductor (mirrors JunctionsService.junctions()).
...
// The geometry the adjacency pass needs: every node's connection points, plus
// the subset that are wire segments (so other points can be tested against them).
interface ConnectionFootprint {
  readonly byId: Map<string, readonly Point[]>;
  readonly wireById: Map<string, WireSegment>;
}

// Reduce each node to its world-space connection points: symbol terminals, or a
// wire's two endpoints (which also go in `wireById` for on-segment tests).
...
// Whether a point sits on an axis-aligned segment (within tolerance): on the
// segment's line, and between its ends. min/max handles either endpoint order;
// padding both bounds by tolerance lets an endpoint-touching point still count.
