 ...
 * or draggable. They're a pure visual side-effect of the geometric connectivity
...
export class JunctionOverlayComponent {
  ...
  /** World-junction positions projected into screen pixels. Re-runs on every
   *  pan/zoom tick and every connectivity-graph change. */
  protected readonly dots = computed<readonly DotScreenPlacement[]>(() => {
    ...
    return this.connectivity.junctions().map((junction) => ({
