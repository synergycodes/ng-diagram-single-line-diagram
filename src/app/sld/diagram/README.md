...
diagram/
  ...
  core/        shared infrastructure (NOT a feature):
    geometry/          pure math + domain types + polyline ops
    ng-diagram-bridge/ adapters over the ng-diagram API (config, actions,
                       pointer-drag, viewport refresh)
    middlewares/       ng-diagram middleware factories (aspect-lock, drag-group,
                       junction port routing)
    mode/              sketch/linking mode + per-mode strategy objects
    keyboard/          diagram keyboard shortcuts
...
| `wire-snap/`        | Auto-align a dragged symbol terminal onto a nearby wire             |
...
```ts
// canvas/diagram.component.ts (sketch)
...
@Component({
  imports: [JunctionOverlayComponent /* … */],
  providers: [...provideJunctions(), ...provideWireSnap() /* … */],
})
export class DiagramComponent {
  private readonly wireSnap = inject(WireSnapController);
  onPaletteItemDropped(e) { this.wireSnap.trySnap(e.node); }
}
```
...
- **Mode strategies** — the sketch/linking difference lives in two objects
  (`core/mode/{sketch,linking}.mode.ts`); the rest of the app reads semantic
  flags off `DiagramModeService.current()` instead of branching on the mode id.
