...
place). Relink therefore depends only on `junctions`, never on `linking` — both
reuse the same attachment service.

**Register:** `...provideRelink()` in the diagram providers; mount
`RelinkOverlayComponent`. (`JunctionAttachmentService` comes from `provideJunctions()`.)
