...
export class JunctionAttachmentService {
  ...
  findNearbyJunction(point: Point, range: number, kind: LinkKind): Node | null {
    ...
    for (const node of this.modelService.getNodesInRange(point, range)) {
      if (!isJunctionNode(node)) continue;
      if ((node.data?.kind ?? 'power') !== kind) continue;
      const centre = junctionWorldCentre(node);
  ...
  // The transaction waits for measurements so callers can reconcile the graph
  // right after awaiting (see RelinkOverlayComponent.resolveDrop).
  async attachToJunction(branch: DroppedBranch, junction: Node): Promise<void> {
    ...
    await this.ngDiagramService.transaction(() => branch.attachTo(junction.id, portId), {
  ...
  // which waits for measurements so callers can reconcile right after awaiting.
  async splitEdgeAtHit(branch: DroppedBranch, hit: EdgeSplitHit): Promise<void> {
