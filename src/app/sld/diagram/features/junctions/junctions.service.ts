...
// Exposes junction dot positions as a reactive signal, derived from world-space
// wire-continuation clusters (3+ ticks) — never stored, always recomputed from
// the current model so it can't drift. The detection algorithm lives in
// graph/junction-detection; this is just its reactive wrapper. Must be provided
// on a component with provideNgDiagram() in scope.
@Injectable()
