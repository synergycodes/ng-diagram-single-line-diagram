...
import { pointBucketKey, SegmentIndex } from '../../../core/geometry/spatial-hash';
import {
  leadBodyEndWorld,
...
// A straight orthogonal run (wire body, lead, or one edge segment) used to
// detect ticks that pass *through* a cluster rather than ending at it.
...
// Derive junction dot positions from raw geometry: collect every wire-end
// "tick" (terminal, wire endpoint, edge endpoint) plus mid-segment crossings,
  ...
  for (const node of nodes) {
    ...
    if (isSymbolNode(node)) {
      ...
      for (const terminalDef of def.terminals) {
        // Sketch-mode connections are power-only — control terminals
