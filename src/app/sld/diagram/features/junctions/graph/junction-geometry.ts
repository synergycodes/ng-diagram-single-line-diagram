...
// Nearest same-kind junction node within `range` of `point`, or null. Pure over
// the supplied nodes so the service just feeds it the live model.
export function findNearbyJunctionNode(
  nodes: readonly Node[],
  point: Point,
  range: number,
  kind: LinkKind,
  ...
  for (const node of nodes) {
    ...
    if ((isJunctionNode(node) ? (node.data?.kind ?? 'power') : 'power') !== kind) continue;
