import type { Edge, Node, NgDiagramModelService, NgDiagramService, Point } from 'ng-diagram';
import {
...
describe('reconcileJunction', () => {
  ...
  describe('collapse (2 branches)', () => {
    ...
    it('rebuilds the reshaped parent route from the two halves instead of resetting it', () => {
      // The parent was reshaped to an L before a branch was dropped on its first
      // segment, splitting it at the junction centre (120,100). Each half keeps
      // its slice of the bends; deleting the branch must re-join them into the
      // original route, not collapse it to a fresh minimal path.
      const model = new FakeModelService();
      const ng = new FakeNgDiagramService();
      // Centre at (120,100): position + size/2 with the 8px junction box.
      const j = { ...makeJunction(), position: { x: 116, y: 96 } } as Node;
      model.nodes.set(j.id, j);
      const data: SldLinkEdgeData = { formerParentId: FORMER_PARENT };
      // Half A: sym-left → junction (left port), a straight lead-in.
      model.edges.set(
        'eA',
        makeEdge('eA', {
          source: 'sym-left',
      ...
      // Half B: junction (right port) → sym-right, carrying the L bend.
      model.edges.set(
        'eB',
        makeEdge('eB', {
          source: j.id,
      ...
      // Seam folds away (the junction sat mid-segment); the L bend survives.
