...
import { RelinkGestureService } from '../../../relink';
import { GRID, PORT_SNAP_PX } from '../../../../core/geometry/constants';
import { edgeKind, portKind, type LinkKind } from '../../../../core/geometry/node-types';
...
// Ghost junction shown while a link-draw or relink gesture is in flight: it
// previews the port-snap or edge-split that a drop at the cursor would produce,
// so mid-edge drop is discoverable. Purely presentational — it reads the live
// gesture state and creates nothing itself.
@Component({
...
export class LinkDropPreviewOverlayComponent {
  ...
  // Excludes the linking gesture's own source node (no self-loop pulse on the
  // port being dragged from) and cross-kind ports (snap preview only for
  // compatible targets).
  ...
  constructor() {
    ...
    // coalesced to one update per frame — each update re-scans every edge in
    // snapTargetPort/edgeHit.
