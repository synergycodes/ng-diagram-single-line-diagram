...
describe('findReshapeableSegments', () => {
  it('returns empty for fewer than 2 points', () => {
    ...
    expect(findReshapeableSegments([], 'anchored', 'anchored')).toEqual([]);
  ...
  it('flags propagateToFreeEnd when source is a junction', () => {
    const r = findReshapeableSegments(
      [
        { x: 0, y: 0 },
        { x: 50, y: 0 },
  ...
  it('flags propagateToFreeEnd when target is a junction', () => {
    const r = findReshapeableSegments(
      [
        { x: 0, y: 0 },
        { x: 50, y: 0 },
  ...
  it('leaves anchor flags off for dangling endpoints (free reshape, both ends shift)', () => {
    ...
    for (const seg of r) {
      ...
      expect(seg.propagateToFreeEnd).toBeNull();
    }
  });
...
describe('reshapeAnchoredSegment', () => {
  ...
  it('keeps the route strictly orthogonal after L-bend insertion (all four cases)', () => {
    const edge = [
      { x: 0, y: 0 },
      { x: 50, y: 0 },
      { x: 50, y: 40 },
