...
// World-space bounds of everything drawn; used to size the viewBox.
...
// A wire node is just a straight line along its orientation, spanning the bbox.
export function renderWireNode(node: Node<SldWireNodeData>): string {
  if (!node.size) return '';
...
// Edge polyline, dashed for control links to match the on-canvas style.
...
// A junction dot, drawn on top of overlapping wire ends.
...
// World-space bounds of everything drawn, used to size the viewBox. Null when
// the model is empty.
...
// Minimal valid SVG for an empty diagram.
...
// Tag anchor point + text-anchor/baseline for the chosen side, offset by TAG_GAP.
...
// The optional 'tag' property as a string, or '' when unset.
