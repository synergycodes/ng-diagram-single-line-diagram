...
describe('stretchPolyline', () => {
  ...
  it('translates the whole polyline rigidly when both endpoints move by the same delta', () => {
    // Mimics alt-drag: both endpoints carried along by the same delta. Every
    // bend should translate uniformly — internal shape preserved.
    const before: Point[] = [
      { x: 0, y: 0 },
      { x: 40, y: 0 },
      { x: 40, y: 100 },
