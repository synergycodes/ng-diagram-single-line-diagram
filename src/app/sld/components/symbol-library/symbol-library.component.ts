...
import { DiagramModeService } from '../../diagram/core/mode/diagram-mode.service';
...
// Drag-source palette: lists wires and registry symbols, grouped by category,
// with search and per-category collapse. Items are dragged onto the canvas.
@Component({
...
export class SymbolLibraryComponent {
  ...
  // All draggable entries: the two built-in wires plus one per registered symbol.
  ...
  readonly groups = computed<readonly CategoryGroup[]>(() => {
    ...
    const hideWires = !this.modeService.current().showsWiresInPalette;
